...
vehicleType = strings(numResults, 1);
...
for i = 1:numResults
    ...
    vehicleType(i) = string(results(i).estimatedVehicleType.label);
    confidence(i) = results(i).confidence;
end

summaryTable = table(imagePath, status, plateText, stateName, vehicleType, confidence);
end
